...
<?php
	...
	{ 
		...
		// Getting the Student of current leave
		...
		if ($status === '2') {
			...
			if ($result) {
				if (filter_var($hodEmail, FILTER_VALIDATE_EMAIL)){
					...
					approve_leave_mail($hodFullname,$studentEmail, "REJECTED");
				} else {
				echo "<script>alert('STUDENT EMAIL IS INVALID. LEAVE APPLICATION REJECTED BUT NO EMAIL SENT');</script>";
				}
		...
		elseif ($status === '1') {
				...
				if ($result) {
					if (filter_var($hodEmail, FILTER_VALIDATE_EMAIL)){
						 if (filter_var($studentEmail, FILTER_VALIDATE_EMAIL)){
							approve_leave_mail($hodFullname,$studentEmail, "APPROVED");
						 } else {
							echo "<script>alert('STUDENT EMAIL IS INVALID. LEAVE APPLICATION APPROVED, BUT NO EMAIL SENT');</script>";
						 }
						...
						<div class="row">
							...
							<div class="col-md-4 col-sm-12">
								<div class="form-group">
									...
									<input type="text" class="selectpicker form-control" data-style="btn-outline-success" readonly value="<?php echo htmlentities($result->Student_ID);?>">
								</div>
							</div>
							<div class="col-md-4 col-sm-12">
								<div class="form-group">
